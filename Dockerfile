...
ARG ZK_DIST=apache-zookeeper-3.5.5
RUN set -x \
    && apt-get update \
    && apt-get install -y openjdk-8-jre-headless wget netcat-openbsd \
        && wget -q "http://www.apache.org/dist/zookeeper/zookeeper-3.5.5/$ZK_DIST.tar.gz" \
    && wget -q "http://www.apache.org/dist/zookeeper/zookeeper-3.5.5/$ZK_DIST.tar.gz.asc" \
